...
- **English (default publication):** `make substack-draft` or **`make sb-en`** (same target)
  ...
  **Body copy:** **`substack.md`** / **`substack.es.md`** is required (see **`.cursor/skills/substack-post/SKILL.md`**). No fallback. Front matter, schedule, tags, and featured image still come from the index file.
