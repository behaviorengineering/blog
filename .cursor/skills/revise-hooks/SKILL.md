...
Increase **pull** on links, list rows, and in-page scans: copy should trigger **curiosity** (open loop, tension, stakes) and still **deliver** on the next screen. This skill governs **hook psychology**, **capture vs pass**, **body headings**, and **cold-read** tests. Type-specific field rules live in **`.cursor/skills/claims-content/SKILL.md`**, **`.cursor/skills/video-content/SKILL.md`**, and **`.cursor/skills/cognitive-memetics-content/SKILL.md`**.
...
**Full lot (flow + hooks + post in one go):** **`.cursor/skills/revise-post/SKILL.md`** (default mode **`rough`**). Use **this skill alone** when you only need list copy and heading hooks.

**Draft or retitle (curiosity gap, 3–5 candidates):** **`.cursor/skills/curiosity-title/SKILL.md`** first, then **this skill** to audit the pick.
...
| **`tags`** | **Attitude and theme** (PascalCase hooks); see claims-content *Tag voice*. They complement the title, not repeat it. Check **`data/tag-register.txt`** and **`.cursor/skills/tag-register/SKILL.md`** before inventing new tag strings. |
...
| **`video`** | **`title`**, **`description`**, **`sowhat`** when set (see **`.cursor/skills/video-content/SKILL.md`** → **Cold-read gate** for bullet rules) |
...
**Voice (MUST):** List-facing fields are also subject to **`.cursor/skills/revise-post/SKILL.md`** → **Step 2** (AI voice) and **Step 5** (negation fluff). Cold-read alone is not enough if the line sounds polished but hollow.
...
- Use **`###`** for **`type: video`** TLDR body sections (and **`### Chapter Guide`**): **`layouts/video/single.html`** already renders **`h1`** title, **`h2`** subtitle, and **`h3`** lead/payoff blocks; body **`##`** would duplicate **`h2`** in the outline. See **`.cursor/skills/video-content/SKILL.md`** → **Body headings**.
- Use **`###`** for **Thoughts** subsections on **`type: claims`** (plain text, **no leading emoji** per **`.cursor/skills/claims-content/SKILL.md`**). Some older **`claims`** essays still use **`##`**; new claims copy MUST open with prose, not **`##`** (see below).
...
The **Thoughts** template already shows a section title above the body. MUST **not** begin claims body markdown with **`##`** (including a repeat of the post **`title`** or a pseudo-article headline). **Fail** hook audit if the first body line is **`## …`**. Open with prose; use **`###`** for the first in-essay hook heading. See **`.cursor/skills/claims-content/SKILL.md`** → **No opening `##` in the body**.
...
After headings pass, apply the same test to sentences and paragraphs under **`.cursor/skills/revise-post/SKILL.md`** → **Step 2** (standalone worth): cut lines that only decorate or restate the hook without new concrete detail.

## Cold-read (naive reader test)

Before publish or before marking **`.cursor/skills/revise-post/SKILL.md`** Step 1 / Step 3 **Pass**:
...
**`type: video`:** MUST also follow **`.cursor/skills/video-content/SKILL.md`** → **Cold-read gate** (**first two `description` bullets** get extra scrutiny).

## Claims posts (`type: claims`)

- **`description`** MUST remain the **Claim** (narrative assertion). See **`.cursor/skills/claims-content/SKILL.md`**.
...
- Follow **`.cursor/skills/video-content/SKILL.md`** for embed, TLDR body, and **Chapter Guide**.
- **`title`** can carry the **watch impulse**; **`description`** lists concrete, honest hooks about what is non-obvious in the talk.
- **MUST** cold-read **`description`** and **`sowhat`** per **Capture vs pass** and **Cold-read** above; extended rules in **`.cursor/skills/video-content/SKILL.md`** → **Cold-read gate**.
- Body **`###`** headings: **Body headings** above.

## Cognitive-memetics (`panel`, `sayings`, hubs)

- Follow **`.cursor/skills/cognitive-memetics-content/SKILL.md`**. Titles often pair **image + phrase**; keep voice consistent with that skill.
