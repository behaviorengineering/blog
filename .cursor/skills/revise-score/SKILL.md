...
For **clarity, compression, flow, and voice** on the prose itself, run **`revise-flow`** (`.cursor/skills/revise-flow/SKILL.md`) before re-scoring.

If the score is below 90, run the **`revise-post`** skill (`.cursor/skills/revise-post/SKILL.md`) for a systematic step-by-step fix.

To run **flow → hooks → post** (and optionally score) in one session, use **`.cursor/skills/revise-post/SKILL.md`** (ask for score as an optional phase).
