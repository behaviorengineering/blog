...
| `carousel.preview` | Thin HTML stub; copy from **`.cursor/skills/carousel-post/templates/carousel.preview`**. **`make serve`**: Hugo serves it as HTML. **`make build`**: renamed to **`carousel.preview.html`** in `public/` for GitHub Pages MIME (same URL without `.html`). |
...
4. **Write `carousel.preview`** from **`.cursor/skills/carousel-post/templates/carousel.preview`** unchanged unless deck JSON filename differs.
...
1. MUST read **`data/short-link-register.txt`** and follow **`.cursor/skills/short-link-register/SKILL.md`** before minting a new path.
