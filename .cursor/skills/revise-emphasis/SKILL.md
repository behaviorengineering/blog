...
**Ship-time sweep:** run **`.cursor/skills/revise-format/SKILL.md`** (emphasis audit + em dash search). Use **this skill** while **authoring** or when only bold rules are needed.
...
| **`type: claims`** **`grounding`** | Construct names and one mechanism anchor; see **Bold for ideas** and **`.cursor/skills/claims-content/SKILL.md`** for digest length. |
...
| **`type: sayings`** **`description`**, **`tldr`**, **`fluff`** | Follow **Sayings** below and **`.cursor/skills/cognitive-memetics-content/SKILL.md`** (Sayings card teaser for **`description`** derives only from **`title`**, **`tldr`**, **`fluff`**). |
...
- Same **Bold for ideas** and **restraint** as English: a few idea-level hooks per block, not saturation. Follow **`.cursor/skills/spanish-translation-content/SKILL.md`** for idiom; follow **this skill** for which **ideas** get bold and how many spans per block.
